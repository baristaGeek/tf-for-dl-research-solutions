...
def word2vec(batch_gen):
    ...
    center_words = tf.placeholders(tf.int32, shape = [BATCH_SIZE], name = 'center_words')
    target_words = tf.placeholders(tf.int32, shape = [BATCH_SIZE, 1], name = 'target_words')

    # Step 2: define weights. In word2vec, it's actually the weights that we care about
    # vocab size x embed size
    # initialized to random uniform -1 to 1
    w = tf.Variable(tf.random_uniform(shape = [VOCAB_SIZE, EMBED_SIZE], minval = -1.0, maxval = 1.0), name = 'weight') #embed_matrix
    ...
    with tf.Session() as sess:
        ...
        for index in xrange(NUM_TRAIN_STEPS):
            ...
            _, loss_batch = sess.run([optimizer, loss], feed_dict={center_words: centers, target_words: target})
